...
import { ArrowRight, CheckCircle2, Sparkles, RotateCcw, XCircle, Loader2, AlertTriangle } from "lucide-react";
import {
...
const LOCATIONS: Record<string, { lat: number; lng: number; city: string }> = {
  ...
  "Tempe, AZ":         { lat: 33.4255, lng: -111.940, city: "Tempe, AZ" },
  ...
  "Las Vegas, NV":     { lat: 36.1699, lng: -115.140, city: "Las Vegas, NV" },
  "Denver, CO":        { lat: 39.7392, lng: -104.990, city: "Denver, CO" },
  "Albuquerque, NM":   { lat: 35.0844, lng: -106.650, city: "Albuquerque, NM" },
  "El Paso, TX":       { lat: 31.7619, lng: -106.485, city: "El Paso, TX" },
  "San Diego, CA":     { lat: 32.7157, lng: -117.161, city: "San Diego, CA" },
  "Salt Lake City, UT":{ lat: 40.7608, lng: -111.891, city: "Salt Lake City, UT" },
};
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">Enter load details — AI evaluates all eligible drivers in &lt;1s.</p>
            </div>
            <span className="text-[10px] font-mono text-primary bg-primary/10 border border-primary/30 px-2 py-1 rounded uppercase tracking-wider">AI Dispatch</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1.5 block">Pickup Location</label>
              <select value={pickup} onChange={(e) => setPickup(e.target.value)} className="w-full bg-input border border-border rounded-md px-3 py-2 text-sm text-foreground focus:outline-none focus:border-primary">
            ...
            <div>
              ...
              <select value={dropoff} onChange={(e) => setDropoff(e.target.value)} className="w-full bg-input border border-border rounded-md px-3 py-2 text-sm text-foreground focus:outline-none focus:border-primary">
                {Object.keys(LOCATIONS).map((loc) => <option key={loc} value={loc}>{loc}</option>)}
              </select>
            </div>
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1.5 block">Load Weight (lbs)</label>
              <input type="number" value={weight} onChange={(e) => setWeight(e.target.value)} className="w-full bg-input border border-border rounded-md px-3 py-2 text-sm text-foreground focus:outline-none focus:border-primary" />
            </div>
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1.5 block">Load Type</label>
              <select value={type} onChange={(e) => setType(e.target.value)} className="w-full bg-input border border-border rounded-md px-3 py-2 text-sm text-foreground focus:outline-none focus:border-primary">
                {TRUCK_TYPES.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
              </select>
            </div>
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1.5 block">Rate ($)</label>
              <input type="number" value={rate} onChange={(e) => setRate(e.target.value)} className="w-full bg-input border border-border rounded-md px-3 py-2 text-sm text-foreground focus:outline-none focus:border-primary" />
            </div>
        ...
        {rec && (
          <>
            <section className="ops-card border-2 border-primary/60 p-6 relative overflow-hidden">
              ...
              <div className="relative">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-[11px] uppercase tracking-widest font-bold text-primary">Recommended Driver</span>
                </div>
                <div className="flex items-start gap-4">
                  <div className="h-14 w-14 rounded-full bg-primary/20 border-2 border-primary/40 flex items-center justify-center text-base font-bold text-primary shrink-0">
                    {rec.driver.name.split(" ").map((n) => n[0]).join("")}
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center gap-3">
                      <h3 className="text-xl font-bold">{rec.driver.name}</h3>
                      <span className="text-[10px] font-mono text-muted-foreground bg-muted px-2 py-0.5 rounded">{rec.driver.truckType}</span>
                    </div>
                    <p className="text-sm text-muted-foreground mt-1">
                      Reason: <span className="text-foreground/90">{rec.reason}</span>
                    </p>
                  </div>
                  <div className="text-right">
                    <div className="text-[10px] uppercase tracking-wider text-muted-foreground">Match Score</div>
                    <div className="text-3xl font-bold text-primary tabular-nums">{Math.round(rec.compositeScore)}</div>
                  </div>
                </div>

                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-5">
                  <Stat label="Deadhead" value={`${rec.deadheadMiles} mi`} />
                  <Stat label="HOS Remaining" value={`${rec.hosRemaining} hrs`} />
                  <Stat label="Est. Cost" value={`$${rec.costPerMile}/mi`} />
                  <Stat label="Est. Profit" value={`$${rec.estimatedTripProfit.toLocaleString()}`} tone="success" />
                </div>
                ...
                {rec.backhaul?.available && rec.backhaul.opportunity && (
                  <div className="mt-5 flex items-center gap-3 bg-success/10 border border-success/30 rounded-md px-4 py-3">
                    ...
                    <p className="text-sm">
                      <span className="font-bold text-success">Return Load Available</span>
                      <span className="text-foreground/80"> — {rec.backhaul.opportunity.distanceToPickup} mi from drop. </span>
                      <span className="font-bold text-success">+${rec.backhaul.opportunity.estimatedRevenue.toLocaleString()} revenue</span>
                    </p>
                  </div>
                )}
            ...
            {alternatives.length > 0 && (
              <section>
                <h3 className="text-sm font-semibold mb-3 text-muted-foreground uppercase tracking-wider">Other Eligible Drivers</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {alternatives.map((d) => (
                    <div key={d.driver.id} className="ops-card p-4 flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-mono text-muted-foreground">{d.driver.truckType}</span>
                        </div>
                        <p className="text-xs text-muted-foreground mt-1">
                          Deadhead {d.deadheadMiles} mi · {d.hosRemaining} hrs HOS · ${d.costPerMile}/mi
                        </p>
                      </div>
                      <div className="text-right">
                        <div className="text-[10px] uppercase text-muted-foreground">Score</div>
                        <div className="text-lg font-bold tabular-nums">{Math.round(d.compositeScore)}</div>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {ineligible.length > 0 && (
              <section>
                <h3 className="text-sm font-semibold mb-3 text-muted-foreground uppercase tracking-wider">Ineligible Drivers</h3>
                <div className="space-y-2">
                  {ineligible.map((d) => (
                    <div key={d.id} className="rounded-lg border border-border bg-muted/20 px-4 py-3 flex items-center gap-3 opacity-80">
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-medium text-foreground/80">{d.name}</p>
                        <p className="text-xs text-danger mt-0.5 truncate">{d.reason}</p>
                      </div>
        ...
        {result && !rec && (
          <section className="ops-card border-2 border-danger/40 p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-danger">No Eligible Drivers</h3>
                <p className="text-sm text-muted-foreground mt-1">{result.message || "All drivers are ineligible for this load."}</p>
                <p className="text-xs text-muted-foreground mt-0.5">{result.totalDrivers} drivers checked · {result.ineligibleCount} ineligible</p>
              </div>
            </div>
            {ineligible.length > 0 && (
...
function Stat({ label, value, tone }: { label: string; value: string; tone?: "success" }) {
  return (
    <div className="rounded-md bg-background/50 border border-border px-3 py-2.5">
      ...
      <div className={`text-base font-bold tabular-nums mt-0.5 ${tone === "success" ? "text-success" : ""}`}>{value}</div>
    </div>
  );
}
